...
with tab_autosave:
    st.header("🤖 Auto-Save Setup")

    st.info(
        "**Before fetching grants**, use the Auto-Save bookmarklet to bulk-save "
        "Instrumentl's recommended matches to your project. Saved grants will then "
        "appear when you fetch in Tab 3.",
        icon="ℹ️",
    )

    st.divider()

    st.subheader("🔖 Browser Bookmarklet — no install required")
    st.write(
